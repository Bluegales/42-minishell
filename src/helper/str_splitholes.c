...
/*   Updated: 2022/04/29 03:03:56 by pfuchs           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		if (last_seperator && !ft_strchr(delims, *str))
		{
			*splits = malloc(1);
			if (!*splits)
				return (1);
			**splits = '\0';
			split_length = create_split(str, delims, splits + 1);
			if (split_length == -1)
				return (1);
			str += split_length - 1;
			splits += 2;
...
static char	**strsplitstrholes(const char *str, const char *delims)
{
	char	**splits;
	char	**it;
	int		count;
	...
	splits = (char **)malloc(sizeof(char *) * (count * 2 + 1));
	if (!splits)
		return (NULL);
	splits[count] = NULL;
...
char	**strsplitholes(char const *s, char c)
{
	char	delim[2];

	delim[0] = c;
	delim[1] = '\0';
	return (strsplitstrholes(s, delim));
}
